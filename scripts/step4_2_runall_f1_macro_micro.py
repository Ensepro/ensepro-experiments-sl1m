...
ensepro = load_json("../analyses/all/analysis_map.json")
metrics = [
    "GOLD",
    "Correto",
    "#respostas",
...
for type in types:
    ...
    for size in sizes:
        for metric in metrics:
            headers.append(type + "[" + str(size) + "][" + metric + "]")
...
# print(df.dtypes)
#
df.to_csv("../analyses/all/csv/f1_scores.csv", sep=";", decimal=',', float_format='%.8f')
